...
export class AccountService {
    ...
    async createTransaction(createTransactionDto, userId): Promise<CategoryResponseDto> {
        ...
        return await this.database.$transaction(async (tsx) => {
            ...
            if (hasAnyBalance === 0 && type !== "INITIAL_CAPITAL") {
                throw new ConflictException("First transaction must be INITIAL_CAPITAL");
            }
            ...
            switch (type) {
                ...
                case 'DEBT_PAID':
                    ...
                    if (type === 'PURCHASE' || type === 'EXPENSES' || type === 'DEBT_PAID') {
            ...
            if (error.code === 'P2002') {
                throw new ConflictException("Please try again. Concurrent transaction detected.");
            }
            throw error;
        });
    }
    ...
    async getCurrentCashBalance(userId): Promise<BalanceDto> {
        ...
        try {
            ...
            const response = await this.database.$transaction(async (tsx) => {

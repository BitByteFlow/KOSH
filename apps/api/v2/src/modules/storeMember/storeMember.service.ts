...
import { StoreMemberResponse, StoreMembersResponse } from "./entities/storeMember.entity";

@Injectable()
export class StoreMemberService {
  constructor(private readonly database: DatabaseService) { }
  async onBoarding(storeId: string, userId: string): Promise<StoreMemberResponse> {
  ...
  async removeMember(storeId: string, requesterId: string, memberId: string): Promise<StoreMemberResponse> {
    // Verify requester is ADMIN of the store
    const requesterMembership = await this.database.prisma.storeMember.findUnique({
    ...
    const store = await this.database.prisma.store.findUnique({ where: { id: storeId } });

...
import { ShoppingCart, History, BookOpen, LogOut, ChevronDown, Store } from 'lucide-react';
    ...
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900">
      ...
      <header className="h-16 px-6 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm sticky top-0 z-50">
        <div className="flex items-center gap-2">
          ...
          <h1 className="font-black text-lg tracking-tight uppercase text-slate-900">KOSH</h1>
        </div>
        ...
        <div className="relative">
          ...
          >
            <div className="text-right hidden sm:block">
              <p className="text-sm font-bold leading-none text-slate-800">{user?.username ?? 'Cashier'}</p>
              <div className="flex items-center justify-end gap-1 mt-0.5">
                <Store size={9} className="text-slate-400" />
            ...
            {showUserMenu && (
              ...
              >
                ...
                <div className="p-4 border-b border-slate-50 bg-slate-50/50">
                  <p className="font-black text-slate-900 leading-tight">{user?.username}</p>
                  <p className="text-xs text-slate-500 mt-0.5">{user?.email}</p>
                  <div className="flex items-center gap-1.5 mt-2">
                    <Store size={11} className="text-primary" />
                    <p className="text-xs font-bold text-primary truncate">{store?.storeName}</p>
                  </div>
                </div>
                {/* Actions */}
                <div className="p-2">
      ...
      <nav className="h-16 bg-white border-t border-slate-200 px-4 flex items-center justify-around">
        ...
        <NavItem to="/sales-history" icon={<History size={20} />} label="Sales" />
        <NavItem to="/daily-transactions" icon={<BookOpen size={20} />} label="Ledger" />
      </nav>
    </div>
  );
};

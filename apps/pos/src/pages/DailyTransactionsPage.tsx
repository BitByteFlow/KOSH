...
import {
  TrendingUp, TrendingDown, DollarSign, Plus, Minus,
  Wallet, ShoppingBag, RefreshCw, Clock, ChevronUp,
...
const TRANSACTION_TYPES = [
  { value: 'WITHDRAWAL', label: 'Withdrawal', icon: ArrowUpRight, color: 'text-red-500', bgColor: 'bg-red-50', badgeClass: 'bg-red-100 text-red-700 border-red-200' },
  { value: 'EXPENSES', label: 'Expense', icon: Minus, color: 'text-orange-500', bgColor: 'bg-orange-50', badgeClass: 'bg-orange-100 text-orange-700 border-orange-200' },
  { value: 'ADDITIONAL_CAPITAL', label: 'Cash In', icon: ArrowDownLeft, color: 'text-green-500', bgColor: 'bg-green-50', badgeClass: 'bg-green-100 text-green-700 border-green-200' },
  { value: 'DEBT_PAID', label: 'Debt Paid', icon: TrendingUp, color: 'text-blue-500', bgColor: 'bg-blue-50', badgeClass: 'bg-blue-100 text-blue-700 border-blue-200' },
  { value: 'ADJUSTMENT', label: 'Adjustment', icon: RefreshCw, color: 'text-purple-500', bgColor: 'bg-purple-50', badgeClass: 'bg-purple-100 text-purple-700 border-purple-200' },
] as const;

const getTypeConfig = (type: string) =>
  TRANSACTION_TYPES.find(t => t.value === type) ?? {
    value: type, label: type, icon: DollarSign,
    color: 'text-slate-500', bgColor: 'bg-slate-50', badgeClass: 'bg-slate-100 text-slate-600 border-slate-200'
  };

const isDebit = (type: string) => ['WITHDRAWAL', 'EXPENSES', 'PURCHASE', 'DEBT'].includes(type);
...
const AddEntryForm: React.FC<AddEntryFormProps> = ({ onSuccess, onCancel }) => {
  const [selectedType, setSelectedType] = useState<string>('EXPENSES');
    ...
    >
      <Card className="border-slate-200 shadow-sm rounded-2xl overflow-hidden">
        <CardHeader className="bg-slate-50 border-b border-slate-100 py-4 px-6">
          <h3 className="font-black text-slate-800 uppercase tracking-tight text-sm">New Transaction Entry</h3>
        </CardHeader>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {TRANSACTION_TYPES.map(({ value, label, icon: Icon, color, bgColor }) => (
                  ...
                  >
                    <div className={`p-1 rounded-lg ${selectedType === value ? 'bg-primary/10' : bgColor}`}>
                      <Icon size={14} className={selectedType === value ? 'text-primary' : color} />
                    </div>
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{format(new Date(), 'EEEE, MMMM d, yyyy')}</p>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight mt-1">DAILY LEDGER</h1>
          <p className="text-slate-500 font-medium mt-1">Record and track today's cash movements.</p>
        </div>
        <Button
        ...
        <Card className="rounded-2xl border-0 bg-linear-to-br from-green-500 to-emerald-600 shadow-lg shadow-green-500/20">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-white/70 text-[10px] font-black uppercase tracking-wider">Cash In</span>
            </div>
            <p className="text-white text-2xl font-black">${totalCashIn.toFixed(2)}</p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl border-0 bg-linear-to-br from-red-500 to-rose-600 shadow-lg shadow-red-500/20">
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-white/70 text-[10px] font-black uppercase tracking-wider">Cash Out</span>
            </div>
            <p className="text-white text-2xl font-black">${totalCashOut.toFixed(2)}</p>
          </CardContent>
        </Card>
        ...
        <Card className={`rounded-2xl border-0 shadow-lg ${netBalance >= 0 ? 'bg-linear-to-br from-blue-500 to-indigo-600 shadow-blue-500/20' : 'bg-linear-to-br from-slate-500 to-slate-700 shadow-slate-500/20'}`}>
          <CardContent className="p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-white/70 text-[10px] font-black uppercase tracking-wider">Net</span>
            </div>
            <p className="text-white text-2xl font-black">{netBalance >= 0 ? '+' : ''}{netBalance.toFixed(2)}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <AnimatePresence>
        {showAddForm && (
          <AddEntryForm onSuccess={handleSuccess} onCancel={() => setShowAddForm(false)} />
        )}
      </AnimatePresence>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-3">
          <h2 className="font-black text-slate-800 uppercase tracking-tighter text-sm">Today's Entries</h2>
          <div className="h-px flex-1 bg-slate-200" />
          <Badge variant="outline" className="text-[10px] font-black px-2 border-slate-200 text-slate-400">
        ...
        {!loading && todayTransactions.length === 0 && (
          <div className="flex flex-col items-center justify-center py-16 bg-white/50 border-2 border-dashed border-slate-200 rounded-3xl text-center">
            ...
            <p className="text-sm text-slate-400 mt-1">Tap "Add Entry" to record your first transaction</p>
          </div>
        )}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <Badge className={`text-[10px] font-black border px-1.5 py-0 ${config.badgeClass}`}>
                      {config.label}
                    </Badge>
                  </div>

...
export function StoreProvider({
	...
	const stores = useMemo<GQL.Store[]>(() => {
		...
		return data.getStores.data.filter(
			(store): store is GQL.Store => store !== null && store.id != null,
		);
	}, [data]);
	...
	const currentStore = useMemo<GQL.Store | null>(() => {
		...
		if (!store && activeStoreId) {
			console.warn(
				`[StoreProvider] Store ${activeStoreId} not found in available stores`,
			);
			return null;
		}
	...
	useEffect(() => {
		...
		const initializeStore = () => {
			if (session?.user?.storeId) {
				...
				toast.warning(
					`Session store ${session.user.storeId} not found in user's stores`,
				);
			}
	...
	const contextValue = useMemo<StoreContextType>(
		...
		[activeStoreId, stores, isLoading, graphqlError, currentStore, switchStore],
	);
